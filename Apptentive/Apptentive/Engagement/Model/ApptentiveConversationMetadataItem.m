...
- (BOOL)isConsistent {
	if (self.state == ApptentiveConversationStateUndefined) {
		ApptentiveLogError(ApptentiveLogTagConversation, @"Conversation metadata item state is undefined.");
		return NO;
	}

	if (self.directoryName.length == 0) {
		ApptentiveLogError(ApptentiveLogTagConversation, @"Conversation metadata directory name is empty.");
		return NO;
	}
	...
	if (self.state == ApptentiveConversationStateAnonymous || self.state == ApptentiveConversationStateLoggedIn) {
		if (self.conversationIdentifier.length == 0) {
			ApptentiveLogError(ApptentiveLogTagConversation, @"Conversation metadata conversation identifier is empty for state %@.", NSStringFromApptentiveConversationState(self.state));
	...
	if (self.state == ApptentiveConversationStateLoggedIn) {
		if (self.userId.length == 0) {
			ApptentiveLogError(ApptentiveLogTagConversation, @"Conversation metadata userId is empty for logged-in conversation.");
			return NO;
		}

		if (self.encryptionKey.length == 0) {
			ApptentiveLogError(ApptentiveLogTagConversation, @"Conversation metadata encryption key is empty for logged-in conversation.");
			return NO;
		}
	}
