...
+ (instancetype)messageWithJSON:(NSDictionary *)json inContext:(NSManagedObjectContext *)context {
	ApptentiveLegacyMessage *message = [ApptentiveLegacyMessage findMessageWithPendingID:json[@"nonce"] inContext:context];
	...
	if (message == nil) {
		message = (ApptentiveLegacyMessage *)[[NSManagedObject alloc] initWithEntity:[NSEntityDescription entityForName:@"ATMessage" inManagedObjectContext:context] insertIntoManagedObjectContext:context];
	}
...
+ (instancetype)newInstanceWithBody:(NSString *)body attachments:(NSArray *)attachments {
	ApptentiveLegacyMessage *result = (ApptentiveLegacyMessage *)[ApptentiveData newEntityNamed:@"ATMessage"];
...
+ (ApptentiveLegacyMessage *)findMessageWithID:(NSString *)apptentiveID inContext:(NSManagedObjectContext *)context {
	ApptentiveLegacyMessage *result = nil;
...
+ (ApptentiveLegacyMessage *)findMessageWithPendingID:(NSString *)pendingID inContext:(NSManagedObjectContext *)context {
	ApptentiveLegacyMessage *result = nil;
...
- (void)updateWithJSON:(NSDictionary *)json {
	...
	if (senderDict != nil) {
		ApptentiveLegacyMessageSender *sender = [ApptentiveLegacyMessageSender newOrExistingMessageSenderFromJSON:senderDict inContext:self.managedObjectContext];
		...
		if (self.attachments.count == 0) {
			...
			for (id tmpAttachment in attachmentsJSON) {
				if ([tmpAttachment isKindOfClass:[NSDictionary class]]) {
					...
					ApptentiveLegacyFileAttachment *attachment = [ApptentiveLegacyFileAttachment newInstanceWithJSON:attachmentJSON inContext:self.managedObjectContext];
			...
			for (NSUInteger i = 0; i < self.attachments.count; i++) {
				ApptentiveLegacyFileAttachment *originalAttachment = [self.attachments objectAtIndex:i];
...
@implementation ApptentiveLegacyMessage (QuickLook)
