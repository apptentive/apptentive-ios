...
@property (strong, nonatomic) ATLargeImageResizer *imageResizer;
@property (strong, nonatomic) NSObject<ATSimpleImageViewControllerDelegate> *delegate;
@property (strong, nonatomic) ATCenteringImageScrollView *scrollView;
...
- (void)setupScrollView {
	...
	if (self.delegate && [self.delegate respondsToSelector:@selector(defaultImageForImageViewController:)]) {
		defaultScreenshot = [self.delegate defaultImageForImageViewController:self];
	}
	if (defaultScreenshot) {
		...
		self.scrollView = [[ATCenteringImageScrollView alloc] initWithImage:defaultScreenshot];
		self.scrollView.backgroundColor = [UIColor blackColor];
		CGSize boundsSize = self.containerView.bounds.size;
		CGSize imageSize = [self.scrollView imageView].image.size;
...
- (void)viewWillDisappear:(BOOL)animated {
	...
	if (self.shouldResign) {
		[self.delegate imageViewControllerWillDismiss:self animated:animated];
		self.delegate = nil;
	}
}
...
- (IBAction)donePressed:(id)sender {
	...
	if ([self respondsToSelector:@selector(dismissViewControllerAnimated:completion:)]) {
		NSObject<ATSimpleImageViewControllerDelegate> *blockDelegate = self.delegate;
...
- (IBAction)takePhoto:(id)sender {
	ATFeedbackAttachmentOptions options = [self.delegate attachmentOptionsForImageViewController:self];
	if (options & ATFeedbackAllowTakePhotoAttachment) {
		[self cleanupImageActionSheet];
		[self dismissImagePickerPopover];
		if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
			self.imageActionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:ATLocalizedString(@"Cancel", @"Cancel button title") destructiveButtonTitle:nil otherButtonTitles:ATLocalizedString(@"Choose From Library", @"Choose Photo Button Title"), ATLocalizedString(@"Take Photo", @"Take Photo Button Title"), nil];
		} else {
			self.imageActionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:ATLocalizedString(@"Cancel", @"Cancel button title") destructiveButtonTitle:nil otherButtonTitles:ATLocalizedString(@"Choose From Library", @"Choose Photo Button Title"), nil];
		}
		
		if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
			[self.imageActionSheet showFromBarButtonItem:self.navigationItem.leftBarButtonItem animated:YES];
		} else {
...
- (void)imageResizerDoneResizing:(ATLargeImageResizer *)resizer result:(UIImage *)image {
	self.activityIndicator.hidden = YES;
	if (image) {
		[self.delegate imageViewController:self pickedImage:image fromSource:self.isFromCamera ? ATFeedbackImageSourceCamera : ATFeedbackImageSourcePhotoLibrary];
	}
	self.imageResizer.delegate = nil;
	self.imageResizer = nil;
	[self setupScrollView];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
	...
	if (actionSheet && self.imageActionSheet && [actionSheet isEqual:self.imageActionSheet]) {
		self.imageActionSheet.delegate = nil;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
	if (self.imageResizer) {
		[self.imageResizer cancel];
		self.imageResizer = nil;
	}
	if (self.delegate) {
		[self.delegate imageViewControllerVoidedDefaultImage:self];
	}
	...
	self.imageResizer = [[ATLargeImageResizer alloc] initWithImageAssetURL:imageURL originalImage:image delegate:self];
	...
	if (self.imagePickerPopover) {
		...
		[self.imageResizer resizeWithMaximumSize:maxSize];
	} else {
...
- (void)popoverControllerDidDismissPopover:(UIPopoverController *)popoverController {
	if (popoverController == self.imagePickerPopover) {
		self.imagePickerPopover.delegate = nil;
	...
	if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
		...
		*/
		if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad && [ATUtilities osVersionGreaterThanOrEqualTo:@"7.0"]) {
			[self.imagePickerPopover presentPopoverFromRect:self.view.frame inView:self.view permittedArrowDirections:0 animated:YES];
		} else {
			[self.imagePickerPopover presentPopoverFromBarButtonItem:self.navigationItem.leftBarButtonItem permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
		}
	} else {
...
- (void)cleanupImageActionSheet {
	if (self.imageActionSheet) {
		self.imageActionSheet.delegate = nil;
		[self.imageActionSheet dismissWithClickedButtonIndex:-1 animated:NO];
		self.imageActionSheet = nil;
	}
}
