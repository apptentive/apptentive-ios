...
#import "ATMessageOld.h"

#import "ATBackend.h"

#define kATMessageCodingVersion 1

@implementation ATMessageOld
...
- (BOOL)isOfMessageDisplayType:(ATMessageDisplayTypeType)typeToCheck {
	BOOL result = NO;
	@synchronized(self) {
		for (NSNumber *typeNumber in displayTypes) {
			ATMessageDisplayTypeType displayType = [typeNumber intValue];
