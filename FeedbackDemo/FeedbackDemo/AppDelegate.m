...
- (void)applicationDidBecomeActive:(UIApplication *)application {
	...
	if ([[ATConnect sharedConnection].apiKey isEqualToString:@"ApptentiveApiKey"]) {
		[[[UIAlertView alloc] initWithTitle:@"Please Set API Key" message:@"This demo app will not work properly until you set your API key in defines.h" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil] show];
	} else if ([[ATConnect sharedConnection].appID isEqualToString:@"ExampleAppID"]) {
		[[[UIAlertView alloc] initWithTitle:@"Please Set App ID" message:@"This demo app won't be able to show your app in the app store until you set your App ID in defines.h" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil] show];
	}
}
